...
/* ── A PAREDE ───────────────────────────────────────────────────────────
   A gravura da parede clareia o fundo da página, e fundo mais claro derruba o
   contraste de TODO texto que corre em cima dele. Não é preciso medir cada
   parágrafo: basta o pior par possível, e ele é único —
...
await prova.goto(`http://localhost:${PORTA}/`, { waitUntil: "networkidle" });
const textos = await prova.evaluate(() => {
  const c = document.createElement("canvas").getContext("2d");
  const emRGB = (v) => {
    c.fillStyle = v;
    c.fillRect(0, 0, 1, 1);
...
console.log(`\nparede · pixel mais claro rgb(${pixelParede.join(",")})`);
for (const [nome, rgb] of Object.entries(textos)) {
  const r = razao(rgb, pixelParede);
  medidos++;
  if (r < MINIMO) reprovados++;
  console.log(
